...
interface AllocationGroupManagerProps {
  onGroupsChange?: () => void;
}

export function AllocationGroupManager({ onGroupsChange }: AllocationGroupManagerProps) {
  ...
  async function fetchGroups() {
    try {
      setGroups(await getAllocationGroups());
      onGroupsChange?.();
    } catch {
      /* ignore */
    }
  ...
  // Fetch groups on mount to show accurate count even when collapsed
  useEffect(() => {
    if (!fetched.current) {
  ...
  async function handleCreate() {
    ...
    try {
      await createAllocationGroup({ name: newName.trim(), color: newColor });
      setNewName("");
  ...
  async function handleUpdate(id: number) {
    try {
      await updateAllocationGroup(id, {
  ...
  async function handleDelete(id: number) {
    ...
    if (
      !confirm(
        `Delete allocation group "${group?.name}"? This will remove all assignments, but holdings will not be deleted.`
      )
    ) {
      return;
    }
    try {
      await deleteAllocationGroup(id);
      setError("");
      await fetchGroups();
    } catch (err) {
  ...
  return (
    <section className="space-y-3">
      ...
      >
        ...
        Allocation Groups ({groups.length})
      </button>
